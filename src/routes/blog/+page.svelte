...
{#if data.blogs.length === 0}
	<div class="flex flex-col container mx-auto w-full min-h-screen p-5">
		<div
			class="flex flex-col-reverse relative md:flex-row p-8 border-4 w-full border-white/40 rounded-xl bg-white/20"
		>
...
{:else}
	...
	<div class="container mx-auto my-4 min-h-screen">
		{#each data.blogs as blog}
			<div
				class="flex flex-col-reverse relative md:flex-row p-4 border-4 w-full h-min border-white/40 rounded-xl bg-white/20"
			>
				<a
					href="/blog/{blog.slug}/"
					class="md:absolute bottom-2 right-2 mt-4 text-blue-300 font-bold text-lg rounded-md hover:bg-white/20 px-4 py-2"
					>Read More >></a
				>
				<a
					href="/blog/{blog.slug}/"
					class="max-w-[354px] shrink-0 relative rounded-xl show-div overflow-hidden flex justify-center items-center"
				>
					<div
						class="visible-div absolute top-0 left-0 rounded-xl z-10 items-center justify-center bg-black/70 w-full h-full"
					>
				...
				<div class="flex flex-col items-center justify-center w-full h-full p-4">
					<div class="relative prose prose-invert h-full">
